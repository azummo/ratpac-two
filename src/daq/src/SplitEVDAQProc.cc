...
void SplitEVDAQProc::BeginOfRun(DS::Run *run) {
  ...
  fEventCounter = 0;
  if (!WasSet("pulse_width")) fPulseWidth = ldaq->GetD("pulse_width");
  if (!WasSet("trigger_threshold")) fTriggerThreshold = ldaq->GetD("trigger_threshold");
  if (!WasSet("trigger_window")) fTriggerWindow = ldaq->GetD("trigger_window");
  if (!WasSet("pmt_lockout")) fPmtLockout = ldaq->GetD("pmt_lockout");
  if (!WasSet("trigger_lockout")) fTriggerLockout = ldaq->GetD("trigger_lockout");
  if (!WasSet("trigger_resolution")) fTriggerResolution = ldaq->GetD("trigger_resolution");
  if (!WasSet("lookback")) fLookback = ldaq->GetD("lookback");
  if (!WasSet("max_hit_time")) fMaxHitTime = ldaq->GetD("max_hit_time");
  if (!WasSet("max_hit_duration")) fMaxHitDuration = ldaq->GetD("max_hit_duration");
  if (!WasSet("trigger_on_noise")) fTriggerOnNoise = ldaq->GetI("trigger_on_noise");
